...
class MCTS:
    ...
    def __init__(self, game, neural_net, args):
        ...
        self.Nsa = {}  # stores the number of times (s, a) has been visited
        self.Ns = {}  # stores the number of times state s has been visited
        self.Ps = {}  # stores the initial policy returned by the neural net
        self.Es = {}  # stores game.getGameEnded for board s
        self.Vs = {}  # stores game.getValidMoves for board s

    def get_action_prob(self, canonical_board, temp=1):
        """
        This function performs numMCTSSims simulations of MCTS starting from the canonical_board.
    ...
    def search(self, canonical_board):
        ...
        until a leaf node is found. The action chosen at each node is the one
        that has the maximum upper confidence bound as described in the paper.
        ...
        if s not in self.Ps:
            # leaf node
            self.Ps[s], v = self.neural_net.predict(canonical_board)
            ...
            if sum_Ps_s > 0:
                ...
                # If you have received dozens or hundreds of these messages, you should pay attention to your NNet and/or training process.
